...
void sendNotification(String fmctoken,String name,String message) async {
  const String serverKey = 'AAAAntBPKPk:APA91bGqz8Pb2So7eP9oWUE8dbIUZ8kmjj9v7o6BI9a5VCB6JI3utBbUWOdPYhaWWocrTXsqrWh5UVTXEYdlBKot1H5KpxYdl2tSLrO3gGcmDrFjhKcLSDXnuTkukAVE1uYyD6pouaj_'; // Replace with your FCM server key
...
class ChatPage extends StatefulWidget {
  ...
  const ChatPage({required this.currentUid,required this.uid,required this.productid,required this.name, Key? key}) : super(key: key);
...
void _handleSendPressed(types.PartialText message) async {
  ...
  try {
    await FirebaseFirestore.instance.collection('productlist').doc(widget.productid.toString()).collection(widget.currentUid).add(textMessage.toJson());
    await FirebaseFirestore.instance.collection('productlist').doc(widget.productid.toString()).collection('message').doc(widget.currentUid).set({'uid':widget.currentUid});
...
void _loadMessages()  {
  FirebaseFirestore.instance.collection('productlist').doc(widget.productid.toString()).collection(widget.currentUid).orderBy('createdAt',descending: true).snapshots().listen(
    (snapshot) {
    ...
    child: SafeArea(
      child: Column(
        ...
        decoration: BoxDecoration(
          ...
          // Add other widgets above the chat container if needed
